 ...
 * `packages/ui-web/src/vocabulary.ts`: both `packages/ui-web` and
 * `packages/ui-native` need these strings at runtime, and a runtime
 * `dependencies` edge from the native library to the web one would contradict
 * the reason `packages/ui-native` exists — the two share a philosophy, not
 * code. `@smog/config` has no runtime dependency of its own (no React anywhere
 * in its tree), which is what already lets it be the shared home for the status
 * list; the labels belong next to it for the same reason.
 * `packages/ui-web/src/vocabulary.ts` re-exports this under its old name so no
