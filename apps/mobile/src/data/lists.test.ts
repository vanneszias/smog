...
/** A 303 whose `Location` is the only thing `postListForm` reads. */
...
describe("the six list writes", () => {
  ...
  it("posts create as a form body and resolves the new list's id from the redirect path", async () => {
    global.fetch = jest.fn(() =>
      redirect("/nl/account/lists/7?notice=created")
    ) as unknown as typeof fetch;
    ...
    expect(url).toContain("/account/lists/create");
    ...
    expect(body.get("name")).toBe("Verjaardag");
    expect(body.get("description")).toBe("Voor het feest");
  });

  it("throws with the code the redirect's error param carries", async () => {
    global.fetch = jest.fn(() =>
      redirect("/nl/account/lists?error=name")
    ) as unknown as typeof fetch;
  ...
  it("rename posts the list's id, name and description", async () => {
    global.fetch = jest.fn(() =>
      redirect("/nl/account/lists/7?notice=renamed")
    ) as unknown as typeof fetch;
    ...
    expect(body.get("name")).toBe("Feestdagen");
  });

  it("delete posts the id and the typed confirmation", async () => {
    global.fetch = jest.fn(() =>
      redirect("/nl/account/lists?notice=deleted")
    ) as unknown as typeof fetch;
    ...
    expect(body.get("id")).toBe("7");
    expect(body.get("confirmName")).toBe("Feestdagen");
  });

  it("add posts the list id and the gesture id", async () => {
    global.fetch = jest.fn(() =>
      redirect("/nl/account/lists/7?notice=added")
    ) as unknown as typeof fetch;
    ...
    expect(body.get("id")).toBe("7");
    expect(body.get("gestureId")).toBe("9");
  });

  it("throws the full-list code without ever posting past the client-side check's caller", async () => {
    // The endpoint itself enforces the cap; this only proves the client
    // reads that specific refusal back off the redirect correctly.
    global.fetch = jest.fn(() =>
      redirect("/nl/account/lists/7?error=full")
    ) as unknown as typeof fetch;
  ...
  it("remove posts the list id and the gesture id", async () => {
    global.fetch = jest.fn(() =>
      redirect("/nl/account/lists/7?notice=removed")
    ) as unknown as typeof fetch;
  ...
  it("share posts the list id and the requested visibility", async () => {
    global.fetch = jest.fn(() =>
      redirect("/nl/account/lists/7?notice=shared")
    ) as unknown as typeof fetch;
    ...
    expect(body.get("id")).toBe("7");
    expect(body.get("visibility")).toBe("shared");
  });
