 ...
 *
 * The referential-integrity rule for `lists.owner` is cascade: a private list
 * has no meaning without its owner, and an ownerless row would strand where no
 * access filter can reach it — `listReadAccess` matches on `owner` or a share
 * token, so a list whose owner column went NULL would be invisible to everyone
 * including admins' own filters and undeletable through the UI.
 *
 ...
 * produces exactly the stranded rows that rule exists to prevent.
 *
 ...
 * `lists`, no hook a later change might add for cleanup. Going through
 ...
 * admin panel can reach: precisely the stranded rows the cascade prevents.
 */
