...
describe("Gestures collection", () => {
  ...
  it("carries the Convex _id as a hidden, read-only, unique legacyId", () => {
    ...
    // contract with that code. Unique so a rerun cannot create a second gesture
    // for one Convex row; indexed because every lookup during import is an
    // equality match on it; hidden and read-only because it is bookkeeping for
    // the importer, not something an editor should see or change; not localized
    // and not required because it exists only on rows the importer created —
    // every gesture made in the admin after cutover has none.
