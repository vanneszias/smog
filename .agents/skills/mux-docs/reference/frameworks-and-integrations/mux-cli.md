...
Listen for Mux webhook events in real-time and forward them to your local development server. Events are stored locally for replay during development.
  ...
  CLI webhook commands are for local development only and provide no delivery guarantees. In production, you must configure a webhook endpoint in the Mux Dashboard that points to your server's webhook URL.
...
| Option | Description |
|--------|-------------|
| --forward-to  | POST received events to a local URL in real-time |
...
| Option | Description |
|--------|-------------|
| --forward-to  | Local URL to POST the example event to (required) |
| --json | Output JSON instead of pretty format |
