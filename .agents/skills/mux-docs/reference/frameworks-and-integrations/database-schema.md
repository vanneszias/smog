...
**Source:** https://mux.com/docs/_guides/integrations/database-schema
...
The first three tables mirror data from the Mux API, kept in sync via webhooks. The videos table is your application's layer on top, linking Mux assets to your users and adding metadata that Mux doesn't manage.

If you're using Supabase or Convex, these tables are created for you automatically. This guide is for teams managing their own database schema.
...
The mux_uploads table tracks direct uploads from your users' browsers. When an upload completes, Mux creates an asset and sets the mux_asset_id.
...
The mux_webhook_events table logs every webhook received from Mux. This is essential for debugging sync issues and ensuring you don't process the same event twice.
...
Use Mux webhooks to keep your database in sync with Mux. When Mux finishes processing a video, delivers an upload, or encounters an error, it sends a webhook to your server.
...
Always verify webhook signatures in production to ensure events are genuinely from Mux. See the webhooks guide for details on signature verification.
...
- Integrate with Supabase: managed Postgres with automatic Mux sync
- Integrate with Convex: real-time database with built-in Mux component
- Listen for webhooks: set up webhook handling for real-time sync
- Secure video playback: protect your content with signed playback IDs
- API reference: full details on all Mux resource fields
