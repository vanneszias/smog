...
Improvements:
 Internal improvements to tracking/reporting the end of pause and seeking states
 Initial video rendition info is now reported via a renditionchange event

Fixes:
 Initial video rendition info matches the presented video track when AVPlayerItem reaches the readyToPlay state
 Fixed: on simulators and iOS/tvOS \<18 devices, duplicate renditionchange events could be produced
 Fixed: view_dropped_frame_count would not be reported when the value is zero
