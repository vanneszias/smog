...
Every decision the controller took during Stage 8.6 without a human, in the order made, each with what it costs if wrong. Copied from the SDD ledger before its workspace was deleted.

- Ruling: T2's className test must assert the class's effect the way `packages/ui-native/src/components/contract.test.tsx` already does for other components (or be dropped if contract.test covers Banner) — a test that only checks the root exists proves nothing — cost if wrong: one weak test survives.
- Ruling: T6 implements gesture_viewed with the plan's stated fallback (a `useRef` holding the last reported id), not the eslint-disable/boolean-dependency variant — Biome's exhaustive-deps rule would reject the latter and the pinned behaviour is "once per id" either way — cost if wrong: none observable.
- Task 1: Ruling: plan-mandated race (a pending loadConsent() overwrites a setConsent/clearConsent made while it is in flight) — fix it with a write generation checked before the load's result is applied — reachable: the Settings switch (Task 7) renders before load and a tap in that window would be silently reverted — cost if wrong: a few lines of state.
- Task 1: Ruling: flash-guard proof — add a test on a fresh module instance (jest.isolateModules) asserting not-loaded before the first read, so the initial `loaded` declaration is guarded despite resetConsentForTests; redo the Step 5 mutation keeping getSnapshot memoised so it fails on the assertion, not by crash — cost if wrong: one extra test.
- Task 2: Ruling: declare react-native-safe-area-context "~5.6.2" in packages/ui-native peerDependencies and devDependencies — every other native library ui-native uses (expo-video, flash-list, nativewind) is a peer, and ~5.6.2 is what apps/mobile and apps/native already pin; resolving it by hoisting alone is an undeclared dependency — cost if wrong: one manifest entry to revert if the inset is later passed as a prop instead.
- Task 4: Ruling: plan-mandated defect — useConsentSync treats `user === null` as signed out, but resolveSessionUser returns null on a failed /users/me with the token still stored, so an offline cold start wiped a signed-in person's decision (Focus 3/5). Rule: the sign-out drop applies only when no token is stored; a stored token with no resolved user is "could not verify" and the pass does nothing. A 401 clears the token (api.ts), so real sign-outs still drop — cost if wrong: a decision survives a sign-out the app could not observe, until the next verified session reconciles it.
- Task 4: Ruling: plan-mandated defect — a queued pass captured userId at enqueue but payloadFetch reads the token at run time, and SessionProvider keeps the old user (loading stays false) while a new token resolves, so A's decision could be POSTed with B's token (a fabricated row, forbidden by Stage 8.5 Ruling 12). Rule: a consent POST may only be sent with the token the current session user was verified with; any pass that finds a different token stored does nothing and waits for the session to settle; plus a timeout on the consent POST so a stalled request cannot hold the queue. Implementer chooses the mechanism (a small session.ts addition exposing the verified token is acceptable) — cost if wrong: a delayed row, never a misattributed one.
- Task 4: Ruling: the loading-guard test must seed a marked decision and assert it survives the loading window.
- Task 6: Ruling: plan-mandated defect — search_performed deduped by query string for the life of the (always-mounted) tab screen, so a genuine repeat search never counts, unlike apps/native which fired per submit. Rule: dedup only re-renders/page loads of one settlement; the guard resets whenever the query changes (including to empty), so settling on a query again after changing it is a new search — cost if wrong: slightly more search events than native's per-submit count for debounced intermediate queries that settle.
- Task 6: Ruling: the search.test timeout is pre-existing CPU/cold-transform-cache slowness (reproduced on the pre-Task-6 baseline at 12.6s under contention); release-check runs every package suite concurrently via turbo. Rule: set an explicit testTimeout in apps/mobile/jest.config.js with a comment citing the measurements, the same way apps/site/vitest.config.mts documents its 30s testTimeout — cost if wrong: a genuinely hung mobile test fails after 30s instead of 5s.
- Task 7: Ruling: plan-mandated defect — AnalyticsRow ignores useConsent().loaded, so a returning person with a stored "granted" sees the switch off until the read resolves (reviewer reproduced it synchronously). Rule: the switch is disabled until loaded (so it cannot be toggled from a value that is not yet the truth), with a test that reads the switch synchronously after render — cost if wrong: the switch is inert for a few milliseconds on cold start.
- Ruling: send route patterns, never concrete paths, to OpenPanel (dynamic segments → [param]) — list ids map one-to-one to accounts; the web sends no paths — cost if wrong: coarser screen analytics.
- Ruling: resolveSessionUser treats a successful /users/me with user === null as definitive sign-out (clear token and verified pair); "could not verify" stays for network/429/5xx only — corrects the premise of the T4 "stored token ≠ sign-out" ruling (Payload answers an expired JWT with 200 {user:null}, not 401) — cost if wrong: a transiently-null server answer signs the person out.
- Ruling: search_performed reports only data that arrived for the current query (not the previous query's retained data) — cost if wrong: none observable.
- Ruling: stack the banner's two buttons vertically, equal width — refusal must be as reachable as acceptance — cost if wrong: a taller banner.
- Ruling: reuse one OpenPanel client for the app's life (clear on withdrawal, keep gating) instead of constructing a new one per grant, to stop leaking AppState listeners — folded into the fix wave because the path fix touches the same module — cost if wrong: none.
- Final review: parked — session.ts re-check and delete are not atomic (a storeToken in that one keychain round-trip would be undone) — Ruling: accept; strictly narrower than the ruled unconditional clear, never leaves a dead token — cost if wrong: a rare sign-in has to be repeated.
- Final review: parked — screen views dedupe per pattern, so two consecutive views of one dynamic route record one screen_view — Ruling: accept; no such navigation exists today and coarser screen analytics was the priced cost of the privacy fix.
