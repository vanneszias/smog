...
- `updateJob` (`queues/utilities/updateJob.js:41-43`) always sets `updatedAt`, so a row a live run is working on keeps a recent `updatedAt`. A scheduled invocation's wall-clock ceiling is fifteen minutes and the cron tick runs inside one (`worker.ts` → `onScheduled` → `ctx.waitUntil`), so no live run can own a row whose `updatedAt` is older than that.
